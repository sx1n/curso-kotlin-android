package com.samuel.motivation.data
...
class Mock {
    ...
    fun getPhrase(categoryId: Int?): String {
        when (categoryId) {
            all -> {
                return listPhrases.random().description
            }

            happy -> {
                val randomNumber = randomizer(0, 5)
                return listPhrases[randomNumber].description
            }

            sunny -> {
                val randomNumber = randomizer(6, 12)
                return listPhrases[randomNumber].description
            }
    ...
    private fun randomizer(start: Int, end: Int): Int {
